...
local function setup_prefs()
	...
	for i, pref in ipairs(prefs) do
		...
		assert(
			type_check(pref.alang, "string table", true),
			"`alang` must be a string or a table of strings: " .. pref_str
		)
		assert(
			type_check(pref.slang, "string table", true),
			"`slang` must be a string or a table of strings: " .. pref_str
		)
...
local function predict_audio()
	...
	if aid == "no" then
		return NO_TRACK
	elseif aid ~= "auto" then
		return audio_tracks[tonumber(aid)]
	end

	local num_tracks = #audio_tracks
	if num_tracks == 1 then
		return audio_tracks[1]
	elseif num_tracks == 0 then
		return NO_TRACK
	end
...
local function set_track(type, id)
	...
	if mp.get_property_number(type) == id then
		return
	end
...
local function is_valid_sub(sub, slang, pref)
	...
	if sub == NO_TRACK then
		...
		if slang == "default" then
			if not sub.default then
				return false
			end
		elseif slang == "forced" then
			if not sub.forced then
				return false
			end
		else
	...
	msg.trace(
		string.format(
			"%s %s whitelist: %s | %s blacklist: %s",
			title,
			passes_whitelist and "passed" or "failed",
			utils.to_string(pref.whitelist),
			passes_blacklist and "passed" or "failed",
			utils.to_string(pref.blacklist)
		)
	)
...
local function find_valid_tracks(manual_audio)
	assert(
		manual_audio == nil or (type(manual_audio) == "table" and manual_audio.id),
		"argument must be an audio track or nil"
	)
	...
	if manual_audio then
		msg.debug("select subtitle for", utils.to_string(manual_audio))
	else
		msg.debug("selecting audio and subtitles")
	end
			...
			if is_valid_audio(audio_track, pref) then
				...
				for _, slang in ipairs(slangs) do
					...
					for _, sub_track in ipairs(sub_track_list) do
						if
							is_valid_sub(sub_track, slang, pref)
							and (
								not pref.condition
								or (
									evaluate_string("return " .. pref.condition, {
...
mp.observe_property("track-auto-selection", "bool", function(_, b)
...
mp.add_hook("on_preloaded", 26, function()
	latest_audio = predict_audio()
end)
...
mp.add_hook("on_preloaded", 30, function()
	...
	if mp.get_property("options/sid") ~= "auto" then
		return
	end
	preload()
end)
